...
<template>
    <div>
        ...
        bg-gradient-to-br from-blue-200 to-purple-200">
            ...
            <div class="max-w-3xl mx-auto pt-12 px-4 sm:px-6 lg:px-8 ">
                ...
                <a href="https://sprig.ch" class="home-rectangle shadow-xl p-8 pr-1 bg-gray-100/30 w-96 hover:shadow-2xl">
                    <img class="w-full -mt-28" src="/sprig.png" alt="SPRIG">
                </a>
                ...
                flex-col justify-around">
                    <div class="font-ligh font-title text-black/60">
                        Sponsored by</div>
                    <img class="" src="/algorand.svg" alt="Algorand logo">
                </div>
            </div>
        </div>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 ">
            <ul class="w-full grid grid-cols-4 items-center gap-4 sm:gap-8 -translate-y-1/2 ">
                <li v-for="stat in stats" :key="stat.label">
                    <router-link to="/search" class="p-4 border-t border-black flex items-center
            ...
            <div class=" flex flex-col space-y-8 mb-12">
                <div v-for="content in texts" :key="content.title" class="even:self-end even:text-right odd:border-l even:border-r border-t-small border-black p-4
                                flex flex-col even:items-end">
                    <h2 class="font-title font-bold text-2xl #text-orange-600">{{ content.title }}</h2>
                    <p class="text-gray-600 w-96 mt-22 ">{{ content.body }}</p>
                    <div class="mt-2">
                        <iframe width="560" height="315" :src="content.video" title="YouTube video player" frameborder="0"
            ...
            <div class="relative pb-16 mx-auto mb-8 flex flex-col items-center bg-white p-4 w-fit text-center">
                ...
                <div class="absolute bottom-0 translate-y-1/2 border border-black p-[2px] transition-all bg-white">
                    ...
                    ">Get started <span
                            class="ml-1 group-hover:translate-x-2 transition-transform">→</span></router-link>
                </div>
            </div>
        </div>
        ...
        bg-gradient-to-tr to-purple-200 from-blue-200">
            <div class="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
                ...
                <div class="flex bg-white -mb-12 border border-black shadow p-6 items-center justify-between mt-10 ">
                    ...
                    <div></div>
                    <a href="mailto:info@sprig.ch" class="flex flex-col items-center mt-2 mr-6 border-2 border-black bg-white py-3 px-6 shadow
        ...
        <div v-if="showEverything" class="mt-20 mb-8 text-gray-500 text-center">
            You've reached the bottom of the page 🎉
            <br>
            It is now time to <router-link to="/search" class="underline hover:text-purple-500">explore</router-link>!
        </div>
        <div v-else class="mt-24 mb-12 text-gray-500 text-sm text-center">
            Made with {{ heart }} by the SPRIG team
        </div>
...
const texts = [
    {
        ...
        body: 'In July 2022, we finished a centralised version of SPRIG, where mathematicians can propose partial proofs of their claims and challenge each other to complete the missing parts when needed. Check out the demo below!',
        video: 'https://www.youtube-nocookie.com/embed/wox6KStzvKw'
    },
    {
        title: 'SPRIG is now decentralised!',
        body: 'In december 2022, we launch the first decentralised version of SPRIG. Anyone can use their ALGOs to add bounties to their claims and challenges.',
        video: 'https://www.youtube-nocookie.com/embed/cDQYljZ4LUg'
    },
    // {
...
// Pick a heart emoji at random
const heart = _.sample(['🧡', '💛', '💚', '💙', '💜', '🤎', '🖤', '🤍']);
...
.border-x-half:before {
    ...
    /* Without this half pixel, it doesn't match perfectly */
    width: 100%;
