    ...
    <div class="group" @click="toggle()" :class="{'cursor-pointer':collapsable}" >
        <div class="flex group w-full " >
            ...
            <v-icon v-if="title" name="md-expandmore-round"
                class="mx-2 transform transition duration-500 text-gray-600 group-hover:scale-125 group-hover:translate-x- group-hover:text-gray-900"
                :class="{ '-rotate-90': collapsed }" />

            <button v-if="buttonText" @click.stop="takeAction()"
                class="border box-border border-slate-300 shadow-sm rounded-md px-4 py-1 ml-auto mr-4 -my-1 self-end transition duration-500 "
                :class="{ 'opacity-0': !collapsed && title }"
                >{{ buttonText }}</button>
        </div>
                ...
                <div class="bg-slate-100 p-4 mt-2 rounded-md flex flex-col shadow-sm cursor-auto">
                    ...
                    <ul v-if="action.type === Action.ROOT_CREATED || action.type === Action.ATTEMPT_CREATED">
                        ...
                        <li>For
                            ...
                            <price
                                :amount="params.upstakes[attempt.height - 1] + params.downstakes[attempt.height - 1]" />
                        </li>
        ...
        <Time :time="action.time" suffix class="block text-xs text-gray-400 mt-2 -mb-4" />
